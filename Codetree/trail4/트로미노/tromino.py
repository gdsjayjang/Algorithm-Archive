...
for i in range(n - 1):
    for j in range(m - 1):
        ...
        v2 = grid[i][j+1]    # 오른쪽 위
        v3 = grid[i+1][j]    # 왼쪽 아래
        v4 = grid[i+1][j+1]  # 오른쪽 아래
        
        # 4칸 중 3칸을 고르는 것은, 1칸을 제외하는 것과 같음
        sum1 = v1 + v2 + v3  # v4 제외 (┌ 모양)
        sum2 = v1 + v2 + v4  # v3 제외 (┐ 모양)
        sum3 = v1 + v3 + v4  # v2 제외 (└ 모양)
        sum4 = v2 + v3 + v4  # v1 제외 (┘ 모양)
